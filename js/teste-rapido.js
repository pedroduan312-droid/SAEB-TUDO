(() => {
  ...
  btnVerQuestoes.addEventListener("click", () => {
    fecharMenuLateral();
    alert(`Questão ${indiceAtual + 1} de ${teste.length}`);
  });
  ...
  function gerarTesteRapido(questoes, qtdPortugues = 3, qtdMatematica = 2) {
    const portugues = questoes.filter((q) => q.materia === "portugues");
    const matematica = questoes.filter((q) => q.materia === "matematica");

    if (portugues.length < qtdPortugues || matematica.length < qtdMatematica) {
      throw new Error("Não há questões suficientes para montar o teste rápido.");
    }

    const sorteadasPortugues = embaralharCopia(portugues).slice(0, qtdPortugues);
    const sorteadasMatematica = embaralharCopia(matematica).slice(0, qtdMatematica);
  ...
  function atualizarCabecalho() {
    contador.textContent = `${indiceAtual + 1}/${teste.length}`;
    progresso.style.width = `${((indiceAtual + 1) / teste.length) * 100}%`;
  }

  function mostrarQuestao() {
    const questao = teste[indiceAtual];
    if (!questao) return;
  ...
  function confirmarResposta() {
    ...
    btnAcao.textContent = indiceAtual === teste.length - 1 ? "finalizar" : "próxima";
    btnAcao.disabled = false;
  }
  ...
  function mostrarTelaFinal() {
    ...
    const porcentagem = Math.round((acertos / teste.length) * 100);

    tituloFinal.textContent = escolherTituloFinal(porcentagem);
    acertosFinal.textContent = `${acertos}/${teste.length}`;
  ...
  async function carregarQuestoes() {
    ...
    try {
      teste = gerarTesteRapido(bancoQuestoes, 3, 2);
    } catch (erro) {
      console.warn("Falha ao montar teste com banco principal. Usando fallback.", erro);
      teste = gerarTesteRapido(QUESTOES_FALLBACK, 3, 2);
    }
